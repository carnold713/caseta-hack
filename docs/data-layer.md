...
```js
import { create, CasetaHome, CasetaDaylight } from '/data/index.js';
...
`web/js/core.js` builds the instances (`DATA`, `HOME`, `DAY`) and every global name the other scripts call now
...
- **Colour vocabulary** (swatches, "Red", "Custom", nearest swatch). Copper Night's colour sheet may carry its
  own swatch set, and a colour's name comes from the nearest swatch. Moves with the colour sheet in phase 3.
