...
(async () => {
  ...
  const watch = page => {
    ...
    page.on('console', m => { if (m.type() === 'error' && !/net::ERR|502|404|401|Failed to load resource/.test(m.text()) && !/\/ui\/font\//.test((m.location() || {}).url || '')) errors.push('console: ' + m.text()); });
  };
  ...
  try {
    ...
    await wait(500);
    const done = await C(() => ({ t: document.querySelector('.ad-card.done .t').textContent, said: document.querySelector('.ad-card.done .ad-said').textContent, go: document.querySelector('.ad-card.done .ad-go').textContent.trim(), steps: document.querySelectorAll('.ad-steps .as.done').length, check: !!document.querySelector('.radar .ad-check') }));
    check(done.t === 'Added Porch Pico' && done.said === `It is in ${roomName} and shows up there in a moment.`, 'Added {name} · It is in {room} and shows up there in a moment.', done);
    ...
    check(off.ember[0] !== 'none' && /ember/.test(off.ember[1]) && off.ember[2].startsWith('4.8s') && /ember/.test(off.ember[3]), 'Home\'s light is one ember, breathing on the ember token (4.8 s)', off.ember);
