...
(async () => {
  ...
  const open = async (opts = {}) => {
    ...
    page.on('console', m => { if (m.type() === 'error' && !/net::ERR|502|404|Failed to load resource|decode downloaded font|OTS parsing/.test(m.text()) && !/\/ui\/font\//.test((m.location() || {}).url || '')) errors.push('console: ' + m.text()); });
    return { ctx, page };
  };
