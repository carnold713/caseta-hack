...
(async () => {
  ...
  check('Arrives in 30 s, kept by the hub, and the chip says so (nothing else repeats it)', (await P()).fade === 30 && (await page.getAttribute('.chip[data-act="scene-fade"][data-s="30"]', 'aria-pressed')) === 'true' && !(await page.$('.arr-head .v')), (await P()).fade);
