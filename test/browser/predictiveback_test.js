...
(async () => {
  ...
  const behind = await C(a => {
    ...
    return { rooms: !!list, hidden: c && c.style.visibility === 'hidden', scale: b && Math.round((b.width / innerWidth) * 1000) / 1000, y: scr && scr.style.transform, under: L && L.nextElementSibling && L.nextElementSibling.id, ids: L ? [...L.querySelectorAll('[id]')].filter(n => !n.closest('svg')).length : -1 };
  }, aid);
  check('behind it is Rooms, the page Back shows, at 0.96, scrolled where it was, the card not there, no ids but its drawings\' own', behind.rooms && behind.hidden && behind.scale === 0.96 && behind.y === `translateY(${-y0}px)` && behind.under === 'screen' && behind.ids === 0, { ...behind, y0 });
