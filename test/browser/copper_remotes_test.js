...
(async () => {
  ...
  // the sentence is the way in: its days are words that open the days, and nothing under it says it again
  check('no rows under the sentence repeating it', !(await page.$('.rt-short')) && !(await page.$('.routine-page .row.kv')));
  check('the days in the sentence are words', (await page.textContent('.rs .tok[data-go$="/days"]')).trim() === 'every day', await page.textContent('.rs .tok[data-go$="/days"]'));
  await tap('.rs .tok[data-go$="/days"]');
  check('which open Which days?', (await page.textContent('#sheet-root h2')) === 'Which days?');
  await tap('#sheet-root .dd[data-d="0"]');
  check('a day there toggles', !(await sc()).days.includes(0), (await sc()).days);
  await tap('#sheet-root .sheet-close');
  check('and the sentence says the new days', (await page.textContent('.rs .tok[data-go$="/days"]')).trim() === 'every day but Sunday', await page.textContent('.rs'));
  await tap('.rs .cl:nth-child(3) .tok:first-of-type');
  check('the underlined word opens What it does', (await page.textContent('#sheet-root h2')) === 'What it does');
