...
async def runner_cases():
    ...
    # back the way it was: each light remembers how it was the moment it went off
    ...
    check("a lit light has nothing to go back to yet", r._last_lit["5"]["off_at"] is None, r._last_lit["5"])
    bridge.devices["6"]["current_state"] = 0; r.zone_changed("6", 0)
    bridge.devices["5"]["current_state"] = 0; r.zone_changed("5", 0)
    check("each one remembers its own level", {d: st["level"] for d, st in r._last_lit.items()} == {"5": 40, "6": 100}, r._last_lit)
    ...
    # the sweep: what went dark together comes back together, and a light nobody has wanted since
    # Tuesday is not part of tonight's press
    bridge.calls.clear()
    r._last_lit["6"]["off_at"] -= 4000
    await r.run_one({"type": "restore", "target": "h:all", "fade": 0})
    check("a light off long before the others stays off", bridge.calls == [("value", "5", 40)], bridge.calls)
    bridge.calls.clear(); r._last_lit.clear()
