"""Level commands coalesce per target: latest wins while one is in flight. Run: python test_lanes.py"""
import asyncio
import json
import os
...
async def main():
    ...
    ran = [(x["target"], x["level"]) for x in a.runner.ran]
    assert ran == [("d:5", 10), ("d:6", 77), ("d:5", 50)], ran
    ids = sorted(r["id"] for r in results if r["type"] == "result")
    assert ids == ["c0", "c1", "c2", "c3", "c4", "other"], ids
    assert all(r["ok"] for r in results), results
    superseded = [r["id"] for r in results if (r.get("detail") or {}).get("superseded")]
    assert superseded == ["c1", "c2", "c3"], superseded
    assert not a._lanes
