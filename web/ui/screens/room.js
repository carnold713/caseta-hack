...
// Out of a wave, the level in it counts to where it is as the house's does on Home (motion.js count), and the words
// crossfade only when how many are on changes, not every time the number moves.
function countHTML0(c, aid, text) {
  const lv = brightLevel(c, aid), m = /^(\d+ on · )(\d+)%$/.exec(text);
  if (!m || !lv) return `<span class="count" data-xf>${c.esc(text)}</span>`;
  return `<span class="count" data-xf="standard" data-xf-look="${c.esc(m[1])}">${c.esc(m[1])}<span data-rlv data-count="${lv}">${lv}</span>%</span>`;
}
...
function wireBright(c, root) {
  ...
  const set = x => {
    ...
    if (n && m && out.firstChild.nodeType === 3) { out.firstChild.textContent = m[1]; n.textContent = m[2]; count.shown(`room:${aid}`, Number(m[2])); }
...
async function runWave(c, el, r, p) {
  ...
  const me = wave = { kind: 'run', id: p.id, aid, t0, x, y, fade, tone: sceneTone(c, p), countWas: countText(c, aid), lands };
