...
export function stepCards(c, list) {
  ...
  const tgt = (i, a) => field('Lights', sel(i, 'target', Array.isArray(a.target) ? [[JSON.stringify(a.target), data.targetName(a.target)], ...targets] : targets, Array.isArray(a.target) ? JSON.stringify(a.target) : a.target));
  return list.map((a, i) => {
    let f = field(`Step ${i + 1}`, sel(i, 'type', Object.entries(REM.ACTION_LABELS), a.type === 'preset' ? 'scene' : a.type));
    switch (a.type) {
      case 'level': f += tgt(i, a) + field('Brightness', sel(i, 'level', [['toggle', 'On or off'], ['on', 'On'], ['off', 'Off'], ...[100, 90, 75, 60, 50, 40, 30, 20, 10, 5].map(v => [v, v + '%'])], a.level)) + field('Fade', sel(i, 'fade', fadeOpts, a.fade ?? '')); break;
      ...
      case 'timer': f += tgt(i, a) + field('After', sel(i, 'minutes', [5, 10, 15, 20, 30, 45, 60, 90, 120].map(m => [m, m + ' minutes']), a.minutes)) + field('Then set to', sel(i, 'level', [[0, 'Off'], [5, '5%'], [10, '10%'], [30, '30%']], a.level || 0)) + field('Fade', sel(i, 'fade', fadeOpts, a.fade ?? '')); break;
    ...
    return `<div class="st-card">${f}<button class="link blue" data-act="st-remove" data-i="${i}">Remove</button></div>`;
  }).join('');
}
