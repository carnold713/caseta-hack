...
// scroll over its first 64 px (0 at rest, 1 from 64 on), which this module writes to #app as --hdr-p (on #app and not
// the page, so it outlives each redraw, and #screen's own style is left to the transitions that move it):
//
...
export function sync() {
  ...
  if (app.style.getPropertyValue('--hdr-p') !== v) app.style.setProperty('--hdr-p', v);
}
