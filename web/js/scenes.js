...
VIEWS.scenes = {
  ...
  body() {
    ...
    if (!mine.length && !theirs.length) return `<button class="tip" data-act="scene-new" style="margin-top:8px"><div class="grow"><span class="cap">Scenes</span><div class="t">Set the lights the way you like them, then save that look</div><div class="d">A remote button can run it later.</div></div><span class="go">${ICON('plus')}</span></button>`;
    let h = '';
...
function openSceneEditor(id, fresh = false) {
  ...
  sheet.open(fresh ? 'What should your lights do?' : 'Edit scene', body, { sub: 'Turn lights on, off or set them to a level.' });
}
