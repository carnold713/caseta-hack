...
VIEWS.settings = {
  ...
  body() {
    ...
    const conn = S.agent.online
      ? `<div class="tip"><div class="grow"><span class="cap">Connection</span><div class="t">Connected to your home</div><div class="d">${plural(nd, 'light')} · ${plural(np, 'remote')}${(info.bridge || {}).host ? ` · bridge at ${esc(info.bridge.host)}` : ''}</div></div><span class="tag green">Connected</span></div>`
    ...
    <div class="card pad0 list">
      ...
      <div class="item"><div class="grow"><div class="t">Connector ${esc(info.version || '?')}${info.commit ? ` <span class="faint small">(${esc(info.commit)})</span>` : ''}</div><div class="d">${info.update_available ? `Update available: ${esc(info.latest)}` : 'Up to date'}</div>${info.update_available ? `<span class="tag">Update available</span>` : ''}</div>${info.update_available ? `<button class="btn sm primary" data-act="update-connector">Update</button>` : ''}</div>
