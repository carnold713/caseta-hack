...
function favTile(t) {
  ...
  return `<button class="tile" data-tgt="${t}" data-act="toggle" data-t="${t}" data-long="open-light"><div class="ic">${ICON(d ? lightIcon(d) : icon, 'sm')}</div><div ${opens ? `class="lopen" data-act="light-open" data-id="${d.device_id}"` : ''}><div class="n">${esc(cap(targetName(t)))}</div><div class="s">${esc(tileSub(t))}</div></div></button>`;
}
...
function lightRow(d) {
  ...
  const icon = opens ? `<button class="lkind" data-act="kind-open" data-id="${id}" title="What kind of light is this?">${ICON(lightIcon(d))}</button>` : ICON(domainIcon(d.domain));
  const head = (sub) => `<div class="row">${icon}<${opens ? `button class="grow lopen" data-act="light-open" data-id="${id}"` : 'div class="grow"'}><div class="n">${esc(d.name)}</div><div class="lv" data-lvl="${id}">${sub}</div></${opens ? 'button' : 'div'}>${fav}`;
