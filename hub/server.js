...
app.get('*', (req, res) => {
  ...
  // A file that is not there (a font, an image, a script: anything with an extension) is a 404, not the app's page:
  // answered with the HTML, a missing font was taken for a broken one and a missing script failed to parse. An
  // address without an extension is one of the app's own routes and still gets the page.
